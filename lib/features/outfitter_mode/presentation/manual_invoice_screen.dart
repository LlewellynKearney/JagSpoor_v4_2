...
class _ManualInvoiceScreenState extends State<ManualInvoiceScreen> {
  ...
  void _selectPackage(Map<String, dynamic> package) {
    setState(() {
      ...
      _basePriceController.text = (package['basePrice'] ?? 0.0).toStringAsFixed(
        2,
      );
    });
  }
  ...
  Future<void> _showPackageForm({Map<String, dynamic>? existingPackage}) async {
    ...
    final nameController = TextEditingController(
      text: isEditing ? (existingPackage['packageName'] ?? '') : '',
    );
    final locationController = TextEditingController(
      text: isEditing ? (existingPackage['packageLocation'] ?? '') : '',
    );
    final descController = TextEditingController(
      text: isEditing ? (existingPackage['packageDescription'] ?? '') : '',
    );
    final priceController = TextEditingController(
      text:
          isEditing
              ? ((existingPackage['basePrice'] ?? 0.0) as double)
                  .toStringAsFixed(2)
              : '',
    );
    ...
    if (isEditing && existingPackage['includedAnimalsJson'] != null) {
      try {
        final decoded = _parseIncludedAnimalsJson(
          existingPackage['includedAnimalsJson']?.toString() ?? '',
        );
        if (decoded['animals'] != null) {
          selectedAnimals = List<String>.from(decoded['animals']);
        }
        if (decoded['slaughterFee'] != null) {
          slaughterFeeController
              .text = ((decoded['slaughterFee'] as num?)?.toDouble() ?? 0)
              .toStringAsFixed(2);
        }
        if (decoded['coldroomFee'] != null) {
          coldroomFeeController
              .text = ((decoded['coldroomFee'] as num?)?.toDouble() ?? 0)
              .toStringAsFixed(2);
        }
        if (decoded['bakkieFee'] != null) {
          bakkieFeeController
              .text = ((decoded['bakkieFee'] as num?)?.toDouble() ?? 0)
              .toStringAsFixed(2);
        }
        if (decoded['phFee'] != null) {
          phFeeController.text = ((decoded['phFee'] as num?)?.toDouble() ?? 0)
              .toStringAsFixed(2);
        }
      } catch (_) {}
    }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Wrap(
                            ...
                            runSpacing: 8,
                            children:
                                _availableAnimals.map((animal) {
                                  final isSelected = selectedAnimals.contains(
                                    animal['name'],
                                  );
                                  return FilterChip(
                                    selected: isSelected,
                            ...
                            child: ElevatedButton(
                              ...
                              onPressed: () async {
                                if (nameController.text.isEmpty) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text('Package name required'),
                                ...
                                final includedAnimalsJson =
                                    _buildIncludedAnimalsJson(
                                      selectedAnimals,
                                      double.tryParse(
                                            slaughterFeeController.text,
                                          ) ??
                                          0,
                                      double.tryParse(
                                            coldroomFeeController.text,
                                          ) ??
                                          0,
                                      double.tryParse(
                                            bakkieFeeController.text,
                                          ) ??
                                          0,
                                      double.tryParse(phFeeController.text) ??
                                          0,
                                    );
  ...
  Map<String, dynamic> _parseIncludedAnimalsJson(String jsonStr) {
    ...
    try {
      ...
      final slaughterMatch = RegExp(
        r'"slaughterFee":([0-9.]+)',
      ).firstMatch(jsonStr);
      if (slaughterMatch != null) {
        result['slaughterFee'] =
            double.tryParse(slaughterMatch.group(1) ?? '0') ?? 0;
      }
      final coldroomMatch = RegExp(
        r'"coldroomFee":([0-9.]+)',
      ).firstMatch(jsonStr);
      if (coldroomMatch != null) {
        result['coldroomFee'] =
            double.tryParse(coldroomMatch.group(1) ?? '0') ?? 0;
      }
                    ...
                    children: [
                      ListTile(
                        ...
                        subtitle: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            if (pkg['packageLocation'] != null &&
                                pkg['packageLocation'].toString().isNotEmpty)
                              Text(
                                '📍 ${pkg['packageLocation']}',
                                style: const TextStyle(
                                  color: Colors.grey,
                            ...
                            if (pkg['packageDescription'] != null &&
                                pkg['packageDescription'].toString().isNotEmpty)
                              Text(
