...
class OutfitterSyncService {
  ...
  final _dirtyRecordsController = StreamController<List<SyncRecord>>.broadcast();
  ...
  Stream<List<SyncRecord>> get dirtyRecordsStream => _dirtyRecordsController.stream;
  ...
  Future<void> checkSyncStatus() async {
    ...
    try {
      ...
      debugPrint('OutfitterSyncService: Found ${dirtyRecords.length} dirty records');
    } catch (e) {
  ...
  Future<bool> markAsSynced(String collection, String docId) async {
    try {
      ...
      // Update local state
      _dirtyRecords.removeWhere((r) => r.id == docId && r.collection == collection);
