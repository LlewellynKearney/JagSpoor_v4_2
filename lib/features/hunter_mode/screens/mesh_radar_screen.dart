  ...
  void _toggleMesh(bool value) async {
    ...
    if (_isMeshActive) {
      ...
      await _meshSync.startBroadcasting(
        hunterName: "Hunter_${DateTime.now().millisecondsSinceEpoch.toString().substring(9)}",
        latitude: -26.5641, 
        longitude: 28.0134,
        lastHarvestTag: "NONE",
      );
      ...
      body: Column(
        children: [
          Container(
            ...
            color: _isMeshActive ? Colors.green.withValues(alpha: 0.2) : Colors.red.withValues(alpha: 0.2),
            child: Text(
              _isMeshActive ? '📡 MESH RECEPTOR ACTIVE: Broadcasting & Discovering...' : '🛑 MESH ADAPTER OFF: Offline Telemetry Disabled',
          ...
          Expanded(
            child: _discoveredPeers.isEmpty
                ? const Center(child: Text('No off-grid team members detected nearby.'))
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          title: Text(peerKey, style: const TextStyle(fontWeight: FontWeight.bold)),
                          subtitle: Text('Lat: ${peerInfo?['lat']} | Lon: ${peerInfo?['lon']}\nLast Harvest Tag: ${peerInfo?['tag']}'),
