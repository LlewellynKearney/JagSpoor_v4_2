...
class OutfitterAnalyticsService {
  static final OutfitterAnalyticsService _instance = OutfitterAnalyticsService._internal();
  ...
  Stream<Map<String, double>> getRevenueSummaryStream(String outfitterId) {
    return _firestore
      ...
      for (final doc in snapshot.docs) {
        ...
        final commission = (data['platformCommissionRands'] ?? 0).toDouble();
  ...
  Stream<List<Map<String, dynamic>>> getFilteredPackagesStream({
    ...
    return _firestore
      ...
      for (final packageDoc in packageSnapshot.docs) {
        ...
        final packageProvince = packageData['province'] as String? ?? 
                               farmData?['province'] as String? ?? 
                               '';
        final packageDistrict = packageData['district'] as String? ?? 
                                farmData?['district'] as String? ?? 
                                '';

        // Apply filters
        bool matchesProvince = province == null || province.isEmpty || 
                              packageProvince.toLowerCase() == province.toLowerCase();
        bool matchesDistrict = district == null || district.isEmpty || 
  ...
  Future<Map<String, Map<String, int>>> getMonthlyBookingStats(
    ...
    for (final doc in snapshot.docs) {
      ...
      final monthKey = '${bookingDate.year}-${bookingDate.month.toString().padLeft(2, '0')}';
      final status = data['status'] as String? ?? '';

      if (stats.containsKey(monthKey)) {
        if (status == 'Approved') {
          stats[monthKey]!['approved'] = (stats[monthKey]!['approved'] ?? 0) + 1;
        } else if (status == 'Declined') {
          stats[monthKey]!['declined'] = (stats[monthKey]!['declined'] ?? 0) + 1;
