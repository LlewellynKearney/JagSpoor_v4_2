...
const List<MaintenanceTask> _handgunSchedule = [
  MaintenanceTask(
    name: 'Field strip, clean & lubricate',
    description:
        'Strip to field level, clean bore, slide rails and feed ramp, then re-lubricate.',
    intervalRounds: 400,
  ),
  MaintenanceTask(
    name: 'Inspect magazine springs',
    description:
        'Check magazine springs and followers for fatigue and weak feeding.',
    intervalRounds: 2000,
  ),
  MaintenanceTask(
    name: 'Detailed strip & deep clean',
    description:
        'Detail strip, remove carbon from internals and inspect for wear.',
    intervalRounds: 2500,
  ),
  MaintenanceTask(
    name: 'Inspect / replace recoil spring',
    description:
        'Recoil springs are typically rated for 3,000–5,000 cycles. Inspect and replace if fatigued.',
    intervalRounds: 5000,
  ),
];
...
const List<MaintenanceTask> _rifleSchedule = [
  MaintenanceTask(
    name: 'Clean bore & chamber',
    description:
        'Run patches/brush through the bore and clean the chamber after the outing.',
    intervalRounds: 200,
  ),
  MaintenanceTask(
    name: 'Field strip, clean & lubricate',
    description: 'Clean the action and bolt, then re-lubricate.',
    intervalRounds: 500,
  ),
  MaintenanceTask(
    name: 'Check & re-torque action screws',
    description:
        'Verify action/stock screw torque to maintain zero and accuracy.',
    intervalRounds: 500,
  ),
  MaintenanceTask(
    name: 'Detailed inspection (bolt, crown, gas system)',
    description:
        'Inspect bolt lugs, muzzle crown and (semi-auto) gas system for wear/carbon.',
    intervalRounds: 2000,
  ),
];
...
const List<MaintenanceTask> _shotgunSchedule = [
  MaintenanceTask(
    name: 'Clean bore & chamber',
    description: 'Clean the bore and chamber after use.',
    intervalRounds: 200,
  ),
  MaintenanceTask(
    name: 'Clean gas system / action',
    description:
        'Gas-operated shotguns accumulate carbon in the ports/piston — clean regularly.',
    intervalRounds: 300,
  ),
  MaintenanceTask(
    name: 'Field strip, clean & lubricate',
    description: 'Strip, clean and re-lubricate moving parts.',
    intervalRounds: 500,
  ),
  MaintenanceTask(
    name: 'Detailed strip & deep clean',
    description: 'Detail strip and inspect O-rings/springs for wear.',
    intervalRounds: 2000,
  ),
];
...
const List<MaintenanceTask> _generalSchedule = [
  MaintenanceTask(
    name: 'Clean bore & chamber',
    description: 'Clean the bore and chamber.',
    intervalRounds: 300,
  ),
  MaintenanceTask(
    name: 'Field strip, clean & lubricate',
    description: 'Strip to field level, clean and re-lubricate.',
    intervalRounds: 400,
  ),
  MaintenanceTask(
    name: 'Detailed strip & deep clean',
    description: 'Detail strip, deep clean and inspect for wear.',
    intervalRounds: 2000,
  ),
];
...
List<MaintenanceTask> scheduleFor(Map<String, String> firearm) {
  final t =
      '${firearm['firearmType'] ?? ''} ${firearm['make'] ?? ''}'.toUpperCase();
  if (t.contains('HANDGUN') || t.contains('PISTOL') || t.contains('REVOLVER')) {
