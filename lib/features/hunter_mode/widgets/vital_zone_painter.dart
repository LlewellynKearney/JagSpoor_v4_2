...
class VitalZonePainter extends CustomPainter {
  ...
  final Offset offset;
  final String
  stanceAngle; // Options: 'Broadside', 'Quartering-Towards', 'Quartering-Away'
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (stanceAngle == 'Quartering-Towards') {
      ...
      organShiftX =
          -12.0 * scale; // Shift vital pocket backward behind rib cage
    }
    ...
    if (species == 'Kudu') {
      canvas.drawCircle(
        Offset((-20 * scale) + organShiftX, -10 * scale),
        r * 1.4,
        lungPaint,
      );
      canvas.drawCircle(
        Offset((-10 * scale) + organShiftX, 20 * scale),
        r * 0.6,
        heartPaint,
      );
      // Bone line transforms laterally based on perspective stance modifiers
      double boneOffset =
          stanceAngle == 'Quartering-Towards'
              ? 15.0
              : (stanceAngle == 'Quartering-Away' ? -15.0 : 0.0);
      canvas.drawLine(
        Offset((-40 * scale) + boneOffset, -60 * scale),
        Offset((-20 * scale) + boneOffset, 30 * scale),
        bonePaint,
      );
    } else if (species == 'Warthog') {
      canvas.drawCircle(
        Offset((-30 * scale) + organShiftX, 15 * scale),
        r * 0.9,
        lungPaint,
      );
      canvas.drawCircle(
        Offset((-25 * scale) + organShiftX, 35 * scale),
        r * 0.4,
        heartPaint,
      );
      double boneOffset =
          stanceAngle == 'Quartering-Towards'
              ? 10.0
              : (stanceAngle == 'Quartering-Away' ? -10.0 : 0.0);
      canvas.drawLine(
        Offset((-45 * scale) + boneOffset, -20 * scale),
        Offset((-30 * scale) + boneOffset, 40 * scale),
        bonePaint,
      );
    } else if (species == 'Impala') {
      canvas.drawCircle(
        Offset((-15 * scale) + organShiftX, 0),
        r * 1.1,
        lungPaint,
      );
      canvas.drawCircle(
        Offset((-5 * scale) + organShiftX, 20 * scale),
        r * 0.5,
        heartPaint,
      );
      double boneOffset =
          stanceAngle == 'Quartering-Towards'
              ? 12.0
              : (stanceAngle == 'Quartering-Away' ? -12.0 : 0.0);
      canvas.drawLine(
        Offset((-35 * scale) + boneOffset, -40 * scale),
        Offset((-15 * scale) + boneOffset, 25 * scale),
        bonePaint,
      );
    } else {
