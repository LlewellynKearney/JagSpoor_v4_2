...
- Flutter SDK installed at `/home/openhands/flutter`. The local checkout is
  **3.44.9 stable** (Dart 3.9, 2026-08-05). The CI pin in
  `.github/workflows/build-and-deploy.yml` is now **3.44.9** (bumped from the
  previous 3.29.1) to match the project's native iOS template — see the CI
  section below for why. Add to PATH: `export PATH="$HOME/flutter/bin:$PATH"`.
...
- `.github/workflows/build-and-deploy.yml` runs: `build-android` (ubuntu),
  ...
  `notify` (needs deploy-firebase, `if: always()`).
- **Flutter pin bumped 3.29.1 → 3.44.9** (root-cause fix for the iOS failures).
  The project's native iOS code (`ios/Runner/SceneDelegate.swift` subclasses
  `FlutterSceneDelegate`; `AppDelegate.swift` originally conformed to
  `FlutterImplicitEngineDelegate` / used `FlutterImplicitEngineBridge` +
  `didInitializeImplicitFlutterEngine`) was generated by a modern Flutter
  template whose APIs do NOT exist in 3.29.1's framework — the Swift compiler
  errored "Cannot find type 'FlutterSceneDelegate'/'FlutterImplicitEngineDelegate'
  in scope". Bumping the pin to 3.44.9 (the local dev version that generated
  these files) makes the native code compile as-is and aligns CI with dev.
  `subosito/flutter-action@v2` with `flutter-version: '3.44.9'` checks out that
  exact stable tag. Android already passed on 3.29.1 (see below) and remains
  green on 3.44.9 (Dart analyze clean; Android embedding backward-compatible;
  `ndkVersion = flutter.ndkVersion` auto-adapts).
- **iOS AppDelegate**: kept in the canonical, version-agnostic
  `FlutterAppDelegate` subclass form that registers plugins via
  ...
  not needed for this single-engine app. `SceneDelegate.swift` is left
  unchanged (compiles on 3.44.9).
...
- **Build incompatibilities verified/aligned**:
  ...
  - Android: `flutter build apk --debug`; `ndkVersion = flutter.ndkVersion`
    ...
    (run 31628707429, `Build Android APK` ✓).
  - iOS: `flutter build ios --simulator --no-codesign` — `--no-codesign`
    ...
    runs inside the Flutter build (~6–8 min).
- **Secret handling**: `deploy-firebase` gates ALL deploy steps behind a
  ...
  still error when the webhook var is unset on some action versions — it is
  non-essential and does not gate the build/deploy).
